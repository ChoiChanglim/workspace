...
public class AesCipher {
	...
	private static final int AES_KEY_LEN = 16 * 2;

	private static byte[] AES_KEY = new byte[AES_KEY_LEN];
	...
	public static final int COL_SIZE = 4, // depth of each column in AES state
											// variable
			NUM_COLS = BLOCK_SIZE / COL_SIZE, // number of columns in AES
												// state variable
	...
	private void AssignAESKeys(String pvKey) {
		int i = 0;
		...
		byte[] bytekey = pvKey.getBytes();

		for (i = 0; i < pvKey.length(); i++) {
			AES_KEY[i + 16] = bytekey[i];
		}
	}
	...
	public String _AESEncrypt(String plainText) {
		int remainLen = 0;
		int posPlain = 0;
		int chiperLen = 0;
		byte[] outbuf = new byte[plainText.getBytes().length + BLOCK_SIZE];
		byte[] inbuf = plainText.getBytes();

		int chiperPos = 0;
		byte[] plain = new byte[BLOCK_SIZE];
		byte[] chiper = new byte[BLOCK_SIZE];

		if (setKey(AES_KEY) < 0) {
			return (new String(""));
		}

		remainLen = plainText.getBytes().length;

		while (true) {
			if (remainLen < BLOCK_SIZE)
				break;

			if (remainLen >= BLOCK_SIZE) {
				System.arraycopy(inbuf, posPlain, plain, 0, BLOCK_SIZE);
				posPlain += BLOCK_SIZE;
				remainLen -= BLOCK_SIZE;
			}
			chiper = encrypt(plain);

			System.arraycopy(chiper, 0, outbuf, chiperPos, BLOCK_SIZE);
			chiperPos += BLOCK_SIZE;
			chiperLen += BLOCK_SIZE;

		}
		if (remainLen > 0)
			System.arraycopy(inbuf, posPlain, plain, 0, remainLen);

		paddingSet(plain, BLOCK_SIZE - remainLen);
		...
		return toHEX(outbuf, chiperLen);
	}


	private void paddingSet(byte[] plain, int count) {
		int startIdx = BLOCK_SIZE - count;

		for (int i = 0; i < count; i++, startIdx++) {
			plain[startIdx] = (byte) count;
		}
	}
	...
	public String decryptString(String chiperText) {
		try {
			return _AESDecrypt(chiperText);
		} catch (Exception e) {
			return (new String(""));
		}
	}
	...
	public String _AESDecrypt(String chiperText) {
		int remainLen = 0;
		int posPlain = 0;
		int plainLen = 0;
		byte[] outbuf = new byte[chiperText.getBytes().length / 2 + 1];
		byte[] inbuf = hex2byte(chiperText);

		int chiperPos = 0;
		byte[] plain = new byte[BLOCK_SIZE];
		byte[] chiper = new byte[BLOCK_SIZE];

		if ((chiperText.getBytes().length % BLOCK_SIZE) != 0) {
			return (new String(""));
		}
		...
		remainLen = inbuf.length;

		while (true) {
			if (remainLen < BLOCK_SIZE)
				break;

			System.arraycopy(inbuf, chiperPos, chiper, 0, BLOCK_SIZE);
			chiperPos += BLOCK_SIZE;
			remainLen -= BLOCK_SIZE;

			plain = decrypt(chiper);

			System.arraycopy(plain, 0, outbuf, posPlain, BLOCK_SIZE);
			posPlain += BLOCK_SIZE;
			plainLen += BLOCK_SIZE;
		}

		int paddingLen = PaddCheck(outbuf, plainLen, BLOCK_SIZE);
		if (paddingLen < 0) {
			return (new String(""));
		}
		plainLen -= paddingLen;

		byte[] outbuf2 = new byte[plainLen];
		System.arraycopy(outbuf, 0, outbuf2, 0, plainLen);

		return (new String(outbuf2));
	}


	public int PaddCheck(byte[] plain, int plainLen, int blockLen) {
		int dPadLen = plain[plainLen - 1];
		if ((dPadLen <= 0) || (dPadLen > blockLen))
			return (-1);
		for (int i = 1; i <= dPadLen; i++)
			if (plain[plainLen - i] != dPadLen)
				return (-2);
		...
		for (i = Nk; i < ROUND_KEY_COUNT; i++) {
			...
			if (i % Nk == 0) {
				...
				t0 = (byte) (S[t1 & 0xFF] ^ rcon[i / Nk]); // nb. constant XOR
															// 1st byte only
				t1 = (S[t2 & 0xFF]);
				t2 = (S[t3 & 0xFF]); // nb. RotWord done by reordering
											// bytes used
				t3 = (S[old0 & 0xFF]);
			} else if ((Nk > 6) && (i % Nk == 4)) {
				// temp = SubWord(temp)
	 ...
	 */
	public static String toHEX(byte[] ba, int length) {
