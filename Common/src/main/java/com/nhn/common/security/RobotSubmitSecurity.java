...
public class RobotSubmitSecurity {
    ...
    /**
     * 클라이언트 아이피획득 (ssl을 L4에 설치한경우도 있어서 만들게 되었다.)
     * @param request
     * @return
     */
    ...
    public static long GetCheckPointTimeStamp(HttpServletRequest request){
        ...
        if(StringUtil.isDigit(encryptAuthorizationTokenCookieValue) == false){
            LOG.warn(CustomException.FLOWTIME_HACK_COOKIE_TOKEN.getMessage()+": "+GetRemoteAddr(request)+", Locale:"+CookieValue(request, "LOCAL"));
    ...
    public static boolean isRobotClient(HttpServletRequest request, long AllowTimeRange, String refererHost){
        ...
        if(headerMap.containsKey("referer") == false){
            LOG.warn(CustomException.EMPTY_HEADER_REFERER.getMessage()+": "+GetRemoteAddr(request) +", Locale:"+CookieValue(request, "LOCAL"));
            throw CustomException.NOT_ALLOWED_USER;
        }
        if(headerMap.get("referer").contains(refererHost) == false){
            LOG.warn(CustomException.INVALID_HEADER_REFERER.getMessage()+": "+GetRemoteAddr(request) +", Locale:"+CookieValue(request, "LOCAL"));
            throw CustomException.NOT_ALLOWED_USER;
        }

        //암호화 쿠키 있는지 확인
        if(headerMap.containsKey(ClientAuthorizationTokenHeaderKey) == false){
            LOG.warn(CustomException.EMPTY_HEADER_TOKEN.getMessage()+": "+GetRemoteAddr(request) +", Locale:"+CookieValue(request, "LOCAL"));
            throw CustomException.NOT_ALLOWED_USER;
        }
        if(RequestUtil.getCookieMap(request).containsKey(EncryptAuthorizationTokenCookieKey) == false){
            LOG.warn(CustomException.EMPTY_COOKIE_TOKEN.getMessage()+": "+GetRemoteAddr(request) +", Locale:"+CookieValue(request, "LOCAL"));
            throw CustomException.NOT_ALLOWED_USER;
        }
        ...
        if(StringUtil.isDigit(headerMap.get(ClientAuthorizationTokenHeaderKey)) == false){
            LOG.warn(CustomException.FLOWTIME_HACK_HEADER_TOKEN.getMessage()+": "+GetRemoteAddr(request) +", Locale:"+CookieValue(request, "LOCAL"));
            throw CustomException.NOT_ALLOWED_USER;
        }
        ...
        Calendar ckc = Calendar.getInstance();
        ckc.setTimeInMillis(checkPoint);
        ...
        /*if(clientPredictionTime > nowServerTime){
            LOG.warn(CustomException.FLOWTIME_HACK_BY_CLIENTPREDICTIONTIME_OVER_THE_SERVERTIME.getMessage()+": "+GetRemoteAddr(request) +", Locale:"+CookieValue(request, "LOCAL"));
            LOG.warn("## "+ "checkPoint:"+checkPoint+"("+sdf.format(ckc.getTime())+")" +", clientFlowTime:+"+clientFlowTime+", clientPredictionTime:"+clientPredictionTime+"("+sdf.format(c.getTime())+"), serverTime:"+ nowServerTime +"("+sdf.format(c2.getTime())+"), result:"+(nowServerTime - clientPredictionTime));
            throw CustomException.NOT_ALLOWED_USER;
        }*/
        if((nowServerTime - clientPredictionTime) > AllowTimeRange){
            LOG.warn(CustomException.FLOWTIME_HACK_BY_ALLOWTIMERANGE_OVER.getMessage()+": "+GetRemoteAddr(request) +", Locale:"+CookieValue(request, "LOCAL"));
            LOG.warn("## "+ "checkPoint:"+checkPoint+"("+sdf.format(ckc.getTime())+")" +", clientFlowTime:+"+clientFlowTime+", clientPredictionTime:"+clientPredictionTime+"("+sdf.format(c.getTime())+"), serverTime:"+ nowServerTime +"("+sdf.format(c2.getTime())+"), result:"+(nowServerTime - clientPredictionTime));
            throw CustomException.NOT_ALLOWED_USER;
        }
